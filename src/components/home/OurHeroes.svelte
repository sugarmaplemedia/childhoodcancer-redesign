<script lang="ts">
    ...
    const handleClick_previous = () => position == 0 ? position = content.length - 1 : position--
    const handleClick_next = () => position == content.length - 1 ? position = 0 : position++
</script>
...
<section class="pt-96">
    ...
    <img 
        ...
        class="
            w-screen xl:w-auto max-w-xl
            absolute right-0 -translate-y-96 -z-30
            rounded-t-3xl xl:rounded-t-none xl:rounded-l-3xl"/>
    {/key}
    <div class="
        w-screen xl:w-auto xl:max-w-4xl py-16 xl:pl-64 xl:pr-16
        relative z-10
        bg-white rounded-t-3xl xl:rounded-t-none xl:rounded-r-3xl
        xl:drop-shadow-blue">
        <div class="
            w-full xl:w-auto pb-8
